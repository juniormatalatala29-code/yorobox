...
const Dashboard: React.FC = () => {
  ...
  // ✅ Vérifie d'abord admin, sinon charge le salon
  useEffect(() => {
    const run = async () => {
      if (!uid) {
        setLoading(false);
      ...
      try {
        ...
        // 2) Sinon continuer sur le salon
        if (!salonRef) {
          setLoading(false);
          return;
        }
        ...
        if (snap.exists()) {
          ...
          setForm((prev) => ({
            ...
            subscriptionType:
              (data.subscriptionType as any) || prev.subscriptionType,
            ...
            catalogue: Array.isArray(data.catalogue)
              ? data.catalogue
              : prev.catalogue,
          }));
        } else {
          // ✅ Créer le doc minimal seulement pour un salon non-admin
          await setDoc(
            salonRef,
            {
              uid,
  ...
  const saveAll = async () => {
    ...
    if (form.whatsapp && !/^\d{8,15}$/.test(form.whatsapp.trim())) {
      return alert(
        "WhatsApp doit être uniquement des chiffres (ex: 243977506981)."
      );
    }
  ...
  const onUploadGallery = async (files?: FileList | null) => {
    ...
    try {
      ...
      setForm((p) => {
        const next = [...(p.gallery || []), ...urls].slice(0, 15);
  ...
  if (!user) {
    return (
      <div
        style={{ minHeight: "100vh", background: bg, color: "#fff", padding: 24 }}
      >
  ...
  // ✅ Si admin, on ouvre le dashboard admin
  if (isAdmin) {
      ...
      >
        ...
        {tab === "profil" && (
          <div style={{ display: "grid", gap: 14 }}>
            <SectionTitle
              ...
              subtitle="Ces infos seront visibles par les clients."
            />
            ...
            <Field label="WhatsApp (chiffres uniquement)">
              <input
                value={form.whatsapp}
                onChange={(e) =>
                  setField("whatsapp", e.target.value.replace(/\s/g, ""))
                }
            ...
            <Field label="Type d’abonnement (catégorie)">
              <select
                ...
                onChange={(e) =>
                  setField("subscriptionType", e.target.value as any)
                }
                style={inputStyle}
              >
        ...
        {tab === "images" && (
          <div style={{ display: "grid", gap: 16 }}>
            <SectionTitle
              title="Images"
              subtitle="Photo profil, bannière et galerie (Cloudinary)."
            />
        ...
        {tab === "tarifs" && (
          <div style={{ display: "grid", gap: 14 }}>
            <SectionTitle
              title="Tarifs"
              subtitle="Les clients verront la liste des prestations + prix."
            />
        ...
        {tab === "catalogue" && (
          <div style={{ display: "grid", gap: 14 }}>
            <SectionTitle
              title="Catalogue"
              subtitle="Produits vendus par le salon (optionnel)."
            />
              ...
              {(form.catalogue || []).map((c, i) => (
                ...
                >
                  <input
                    value={c.produit}
                    onChange={(e) =>
                      updateCatalogue(i, "produit", e.target.value)
                    }
              ...
              <div style={{ padding: 16, display: "grid", gap: 10 }}>
                ...
                {!!form.horaires && (
                  <div style={{ opacity: 0.92 }}>
                    <span style={{ color: gold, fontWeight: 900 }}>Horaires:</span>{" "}
                    {form.horaires}
                  </div>
                )}
